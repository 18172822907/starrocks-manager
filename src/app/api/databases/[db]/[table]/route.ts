import { NextRequest, NextResponse } from 'next/server';
import { executeQuery } from '@/lib/db';

export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ db: string; table: string }> }
) {
  try {
    const sessionId = request.nextUrl.searchParams.get('sessionId');
    if (!sessionId) {
      return NextResponse.json({ error: 'Session ID required' }, { status: 400 });
    }

    const { db, table } = await params;
    const fullName = `\`${db}\`.\`${table}\``;
    const limit = Math.min(Math.max(parseInt(request.nextUrl.searchParams.get('limit') || '10', 10) || 10, 1), 1000);

    // Fetch multiple details in parallel
    const [schema, createTable, partitions, preview] = await Promise.all([
      executeQuery(sessionId, `DESC ${fullName}`, undefined, 'databases').catch(() => ({ rows: [], fields: [] })),
      executeQuery(sessionId, `SHOW CREATE TABLE ${fullName}`, undefined, 'databases').catch(() => ({ rows: [], fields: [] })),
      executeQuery(sessionId, `SHOW PARTITIONS FROM ${fullName}`, undefined, 'databases').catch(() => ({ rows: [], fields: [] })),
      executeQuery(sessionId, `SELECT * FROM ${fullName} LIMIT ${limit}`, undefined, 'databases').catch(() => ({ rows: [], fields: [] })),
    ]);

    // Extract DDL
    const ddl = createTable.rows[0]
      ? (createTable.rows[0] as Record<string, string>)['Create Table'] || ''
      : '';

    return NextResponse.json({
      database: db,
      table,
      schema: schema.rows,
      ddl,
      partitions: partitions.rows,
      preview: { rows: preview.rows, fields: preview.fields },
    });
  } catch (err) {
    return NextResponse.json(
      { error: err instanceof Error ? err.message : String(err) },
      { status: 500 }
    );
  }
}
